...
export function usePerformanceMonitoring(options: UsePerformanceMonitoringOptions = {}): UsePerformanceMonitoringResult {
  ...
  const endProfile = useCallback((profileId: string) => {
    ...
    if (detectRegressions && metrics.times.length >= 5) {
      const recentAverage = metrics.times.slice(-3).reduce((a: number, b: number) => a + b, 0) / 3;
      const baselineAverage = metrics.times.slice(0, -3).reduce((a: number, b: number) => a + b, 0) / (metrics.times.length - 3);
