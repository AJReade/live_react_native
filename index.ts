// LiveReact Native - Phoenix LiveView + React Native Integration
// Export all public APIs

// Core Hooks
export { useLiveView } from './js/hooks/useLiveView';
export { useAdvancedUpdates } from './js/hooks/useAdvancedUpdates';
export { usePerformanceMonitoring } from './js/hooks/usePerformanceMonitoring';

// Client Components
export { LiveViewChannel } from './js/client/LiveViewChannel';

// Type Definitions
export type {
  LiveViewOptions,
  LiveViewJoinOptions,
  LiveViewLeaveOptions,
  PushEventOptions,
  ConnectionState,
  LiveViewAssignsUpdate
} from './js/types';

export type {
  UseAdvancedUpdatesOptions,
  UseAdvancedUpdatesResult
} from './js/hooks/useAdvancedUpdates';

export type {
  UsePerformanceMonitoringOptions,
  UsePerformanceMonitoringResult
} from './js/hooks/usePerformanceMonitoring';

// Version
export const version = '1.0.0';